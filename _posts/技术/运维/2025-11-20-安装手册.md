...
![](/assets/img/mysql-windows2.png)

添加PATH

![](/assets/img/mysql-windows3.png)
...
该版本默认源中就是mysql8，直接使用

```
apt install mysql-server
```

安全初始化，设置密码等级，删除匿名用户等
...
```
sudo mysql -u root -p 回车进入
```

root用户默认只能本地登录，为了安全起见不做修改，只改密码
...
然后在`/etc/mysql/mysql.conf.d/mysqld.cnf`中注释掉`bind-address 127.0.0.0`
...
登录mysql创建新用户，并开启远程登录

```
CREATE USER 'ligen'@'%' IDENTIFIED WITH mysql_native_password BY 'password';
GRANT create,drop,select,insert,update,delete ON *.* TO 'ligen'@'%';
GRANT ALL database.tablename TO 'username'@'%'; -- 授予权限
...
> docker run -p 3306:3306 --privileged=true -e MYSQL_ROOT_PASSWORD=123456 -e TZ=Asia/Shanghai -v D:/Program/Docker/mount/mysql/data:/var/lib/mysql/:rw -v D:/Program/Docker/mount/mysql/logs:/var/log/mysql/:rw -v D:/Program/Docker/mount/mysql/conf/:/etc/mysql/ --name mysql_tinode --restart=always -d mysql:8.0.21

遇到的问题：

1. 直接挂载/var/lib/mysql目录出现权限问题

> 先不挂载启动一次，然后将这个目录的文件移动到宿主机，再创建新容器进行挂载

2. mysql8版本使用工具连接时出现version mismatch

> 将/etc/mysql/my.cnf配置文件赋值出来，在[mysqld]中加入mysqlx=off
>
> 然后重新启动容器
...
执行命令
```shell
docker run --restart=always -p 6379:6379 --name redis_tinode \
-v D:/Program/Docker/mount/redis/redis_tinode/redis_tinode.conf:/etc/redis/redis.conf \ 
-v D:/Program/Docker/mount/redis/redis_tinode/data:/data \
...
在配置文件中设置密码和是否远程连接`/etc/redis/redis.conf`
...
logs plugins data
```
config也可以挂载，不过需要有配置文件
如果自己有可以直接设置，没有的话先运行容器，从容器再复制出来
```shell
docker cp container:/usr/share/elasticsearch/config .
```

elasticsearch在docker中的user:group是`1000:1000`
...
> ES_JAVA_OPTS="-Xms84m -Xmx512m" 设置内存
> -v 挂载设置，官方镜像相关目录都在`/usr/share/elasticsearch`
> 版本elasticsearch:8.11.3

经过测试，8.19.7版本在启动时出现错误
> seccomp unavailable: CONFIG_SECCOMP not compiled into kernel, CONFIG_SECCOMP and CONFIG_SECCOMP_FILTER are needed

尝试多种方式均失败，以下是尝试方式。于是降低版本至8.11.3
> 1. --security-opt seccomp=unconfined
> 2. bootstrap.memory_lock: false
以上方法均不行，最后将将配置文件中的安全配置注释，至此启动成功

### 设置认证
```shell
docker exec -it elasticsearch8.11.3 /bin/bash # 进入容器
/bin/elasticsearch-reset-password -u elastic -i # elastic默认用户名，设置密码就好
/bin/elasticsearch-users useradd ligen # 创建新用户，kibana用
```
接下来可以在`http://url:9200`通过上述用户名密码进行访问
### 设置分词插件
...
```
pipeline中是logstash.conf，config中是logstash.yml
...
```
http.host: "0.0.0.0"
xpack.monitoring.elasticsearch.hosts: [ "http://elasticsearch8.11.3-2:9200" ] #容器名或者服务名
# 安全配置
...
output {
  elasticsearch {
      hosts  => ["http://elasticsearch8.11.3-2:9200"]
      user => elastic
      password => 123456
      index  => "logs-%{+YYYY.MM}"
      codec  => "json"
...
遇到问题：使用服务名称，logstash配置使用es服务名无法连接，换成ip可以启动
```
启动容器
```shell
docker run --name logstash8.11.3 --net elk -p 5044:5044 -p 9600:9600 \
	--privileged=true \
	-e ES_JAVA_OPTS="-Duser.timezone=Asia/Shanghai" \
	-v /volume5/docker/program/prod/elk/logstash/pipeline/logstash.conf:/usr/share/logstash/pipeline/logstash.conf \
	-v /volume5/docker/program/prod/elk/logstash/config/logstash.yml:/usr/share/logstash/config/logstash.yml \
	-d logstash:8.11.3
```
...
filebeat.inputs:
 ...
 - type: log
   ...
   scan_frequency: "10s"
   paths:
     - /usr/share/filebeat/business/schola/yudao-server-warn*.log
     - /usr/share/filebeat/business/schola/schola-server-warn*.log
   fields:
     environment: schola_prod
     version: 4.0.1
     log_level: warn  # 添加日志级别字段
   fields_under_root: true  # 重要：让字段出现在根级别
 - type: log
   encoding: utf-8
   enabled: true
   ...
   scan_frequency: "10s"
   paths:
     - /usr/share/filebeat/business/schola/yudao-server-error*.log
     - /usr/share/filebeat/business/schola/schola-server-error*.log
   fields:
     environment: schola_prod
     version: 4.0.1
     log_level: error  # 添加日志级别字段
   fields_under_root: true  # 重要：让字段出现在根级别
...
output.elasticsearch:
  ...
  # 根据日志级别指定不同的索引
  indices:
    - index: "schola-info-%{+yyyy.MM.dd}"
      when.equals:
        log_level: "info"
    - index: "schola-warn-%{+yyyy.MM.dd}"
      when.equals:
        log_level: "warn"
    - index: "schola-error-%{+yyyy.MM.dd}"
      when.equals:
        log_level: "error"
```
执行命令
...
docker run --name filebeat --net elk\
  ...
  -v /volume5/docker/program/prod/elk/filebeat/config/filebeat.yml:/usr/share/filebeat/filebeat.yml \
...
Exiting: error loading config file: config file ("filebeat.yml") can only be writable by the owner but the permissions are "-rw-rw-rw-" (to fix the permissions use: 'chmod go-w /usr/share/filebeat/filebeat.yml')
```
只给配置文件444权限